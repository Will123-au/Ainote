...
"Format as youtube_video|enhance|meeting_note|research_paper" targets the **current file** (\`<editor_context><file>\` or recent file in chat) unless stated otherwise. Apply via \`modifyDocumentText\` / \`addTextToDocument\`. youtube_video: frontmatter, embed, summary from transcript (\`getYoutubeVideoId\` if needed). Don't ask what to format or for confirmation — start immediately using the path from editor context.`;
